...
gulp.task('sprite', function() {
    var spriteData = gulp.src('dist/img/sprites/*.png').pipe(spritesmith({imgName: 'sprite.png', cssName: 'sprite.scss', algorithm: 'binary-tree'}));
    ...
    setTimeout(function() {
        ...
        return gulp.src('src/scss/temp', {read: false}).pipe(clean());
    }, 1000)
});
...
gulp.task('pug', function() {
    gulp.src('./src/templates/*.pug').pipe(plumber({})).pipe(pug({pretty: true})).pipe(gulp.dest('./dist/'));
    setTimeout(function() {
        reload();
    }, 800)
});
...
gulp.task('sass', function() {
    ...
    })).pipe(sass({includePaths: require('bourbon').includePaths, outputStyle: 'compressed'})).pipe(autoprefixer({browsers: ['last 3 versions'], cascade: false})).pipe(sourcemaps.write('./')).pipe(gulp.dest('./dist/css'));
});

gulp.task('connect', function() {
    connect.server({port: 1337, root: 'dist', livereload: true});
});
//Sprite Generator
